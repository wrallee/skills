...
- **Audiveris `-export` triggers full pipeline (RHYTHMS→PAGE).** Calling `audiveris -batch -export` on a LINKS-only `.omr` will run all remaining steps (RHYTHMS, PAGE, SCORES), including MeasureFixer which may `system.removeStack()` broken/empty measures. To extract clef/key/time without losing measures, parse `sheet#1.xml` directly from the `.omr` zip using `audiveris/sheet_to_musicxml.py` rather than relying on Audiveris's MusicXML export.
...
- **Next step (Audiveris):** See `AUDIVERIS-NEXT.md` at skill root — clef/key signature extraction via Audiveris OMR, measure reconstruction plan. Supporting files in `audiveris/`: `sheet_to_musicxml.py` (LINKS sheet#1.xml → MusicXML converter), `musescore-harmony-import-path.md`, `musescore-import-path-chord-symbols.md` (MuseScore source analysis for harmony import).
